# PhotoLuxEV

A simple android app where a user can input a Lux value measured by lower cost light meters.
The app then converts the Lux value to an exposure value (EV) useful in photography.

Future feature: To be able to translate exposure value into exposure triangle suggestions.

## Built With

* [Android Studio](https://developer.android.com/studio) - The IDE
* [Kotlin](https://kotlinlang.org/) - The programming langauge

## Contributing

Please read [CONTRIBUTING.md](https://github.com/Keacoder/PhotoLuxEV/blob/master/CONTRIBUTING.md) for details on our code of conduct and welcomed contributions.

## Author/s

* **Keacoder** - *Initial work* - [Keacoder](https://github.com/Keacoder)

## License

This project is licensed under the GNU GPLv3 License - see the [LICENSE.md](LICENSE.md) file for details

If you have any questions contact Keacoder at keacoder@protonmail.com.
